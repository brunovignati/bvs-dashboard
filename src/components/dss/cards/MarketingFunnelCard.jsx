...
function Stage({ label, source, value, srcState, color }) {
  ...
  return (
    <div className="rounded-xl border border-border p-3" style={{ background: `${color}0d` }}>
...
export default function MarketingFunnelCard({ delay }) {
  ...
  // 1) Notoriedad (Metricool) — audiencia social (seguidores última fila por red)
  ...
  // 2) Tráfico (GA4) — métrica detectada, suma últimos 30 registros
  ...
  // 3) Conversión + 4) Revenue (Connectif) — últimos 30 días
  ...
  return (
    ...
    >
      <div className="space-y-1.5">
        <Stage label="1 · Notoriedad" source="Metricool" value={socialState==="green"?`${fmtNumber(followers)} seguidores`:"pendiente de datos"} srcState={socialState} color="#E1306C" />
        <div className="flex justify-center"><ChevronDown className="w-4 h-4 text-muted-foreground" /></div>
        <Stage label={`2 · Tráfico (${trafficLabel})`} source="GA4" value={trafficState==="green"?`${fmtNumber(traffic)} / 30d`:"pendiente de datos"} srcState={trafficState} color="#F59E0B" />
        <div className="flex justify-center"><ChevronDown className="w-4 h-4 text-muted-foreground" /></div>
        <Stage label="3 · Conversión" source="Connectif" value={connState==="green"?`${fmtNumber(purchases)} compras`:"pendiente de datos"} srcState={connState} color="#3B82F6" />
        <div className="flex justify-center"><ChevronDown className="w-4 h-4 text-muted-foreground" /></div>
        <Stage label="4 · Revenue" source="Connectif" value={connState==="green"?fmtCurrency(revenue):"pendiente de datos"} srcState={connState} color="#10B981" />
      </div>
    </EvidenceCard>
  );
}
