...
export default function WebStickyCard({ delay }) {
  ...
  const rows = [...data].filter(s => s.workflow).sort((a,b)=>(b.revenue||0)-(a.revenue||0)).slice(0,8)
  ...
  return (
    <EvidenceCard
      ...
      maturity="amber"
      actions={[
        { verb: "escalar", rationale: best ? `Mantén y replica la variante ganadora: "${best.name}".` : "Prioriza las piezas con conversión probada." },
        { verb: "detener", rationale: "Retira los sticky con conversión 0 sostenida (ocupan espacio sin captar)." },
      ]}
      delay={delay}
      note="Fuente: Connectif · sticky (acumulado sin fecha). Valor medido por captación, no solo venta directa."
    >
      {hasData && (
        <div className="h-48">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={rows} layout="vertical" margin={{ top:4, right:16, left:4, bottom:0 }}>
              ...
              <Tooltip formatter={(v)=>[fmtCurrency(v),"Revenue"]} labelFormatter={(l,p)=>p?.[0]?.payload?.full||l} labelStyle={{ fontSize:10 }} />
              <Bar dataKey="revenue" radius={[0,4,4,0]}>
                {rows.map((r,i)=><Cell key={i} fill={r.inactive?"hsl(220,13%,70%)":i===0?"hsl(160,84%,39%)":"hsl(217,91%,60%)"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
