        ...
        <div className="h-56">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={top} layout="vertical" margin={{ top: 4, right: 12, left: 4, bottom: 0 }}>
              ...
              <YAxis type="category" dataKey="short" width={140} tick={{ fontSize: 8, fill: "hsl(32,7%,48%)" }} axisLine={false} tickLine={false} />
              <Tooltip formatter={(v) => [fmtCurrency(v), "Revenue"]} labelFormatter={(l, p) => p?.[0]?.payload?.name || l} labelStyle={{ fontSize: 10 }} />
              <Bar dataKey="revenue" radius={[0, 4, 4, 0]}>
                {top.map((w, i) => <Cell key={i} fill={w.channel === "Push" ? "hsl(30,72%,66%)" : i === 0 ? "hsl(30,72%,66%)" : "hsl(16,79%,57%)"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
