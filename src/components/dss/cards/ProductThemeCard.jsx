...
export default function ProductThemeCard({ delay }) {
  ...
  if (!hasData) {
    return (
      <EvidenceCard question="PR-2 · ¿Qué temáticas de producto funcionan mejor? (proxy)" answer="Sin señal suficiente" answerTone="neutral"
  ...
  return (
    <EvidenceCard
      question="PR-2 · ¿Qué temáticas de producto funcionan mejor? (proxy)"
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={rows} layout="vertical" margin={{ top:4, right:16, left:4, bottom:0 }}>
            ...
            <Bar dataKey="revenue" radius={[0,4,4,0]}>
              {rows.map((_,i)=><Cell key={i} fill={i===0?"hsl(160,84%,39%)":"hsl(217,91%,60%)"} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
