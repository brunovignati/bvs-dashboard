              ...
              <AreaChart data={emailChartData} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                <defs>
                  <linearGradient id="subGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%"  stopColor="hsl(200,85%,54%)" stopOpacity={0.35} />
                    <stop offset="95%" stopColor="hsl(200,85%,54%)" stopOpacity={0.02} />
                  </linearGradient>
                ...
                <Area type="monotone" dataKey="Suscritos"   stroke="hsl(200,85%,54%)" fill="url(#subGrad)"   strokeWidth={2} />
            ...
            <div className="h-52 mb-3">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={pushChartData} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend iconType="circle" iconSize={6} wrapperStyle={{ fontSize: 10 }} />
                  <Area type="monotone" dataKey="Suscriptores" stroke="hsl(200,85%,54%)" fill="url(#pushSubGrad)" strokeWidth={2} dot={{ r: 3 }} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
                ...
                <BarChart data={pushChartData} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                  ...
                  <Bar dataKey="Cambio" name="Variación" radius={[2, 2, 0, 0]} maxBarSize={24}>
                    {pushChartData.map((d, i) => (
                      <Cell key={i} fill={d.Cambio >= 0 ? 'hsl(200,85%,54%)' : 'hsl(200,90%,38%)'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
