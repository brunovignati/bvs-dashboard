...
export function useMiCompiDaily() {
  return useQuery({
    queryKey: ['mi_compi_daily'],
    queryFn: () => fetchTable('mi_compi_daily', 'dia', true, 1000),
    initialData: [],
  })
}

export function useMiCompiCumulative() {
  return useQuery({
    queryKey: ['mi_compi_cumulative'],
    queryFn: () => fetchTable('mi_compi_cumulative', 'dia', true, 1000),
    initialData: [],
  })
}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select('email, nombre_1, especie_1, raza_1, num_compis, first_seen_at')
